...
exports.Jianpu = void 0;
// abc2svg - Jianpu module
var Abc_1 = require("../src/Abc");
var abc2svg = __importStar(require("../src/abc2svg"));
var abc;
var Jianpu = /** @class */ (function () {
    function Jianpu(abc_) {
        abc = abc_;
    }
    // don't calculate the beams
    Jianpu.prototype.calc_beam = function (of, bm, s1) {
        if (!s1.p_v.jianpu)
            return of(bm, s1);
        // return 0
    };
    // adjust some symbols before the generation
    Jianpu.prototype.output_music = function (of) {
        var p_v, v, C = abc2svg.C, cur_sy = abc.get_cur_sy(), voice_tb = abc.get_voice_tb();
        // handle the overlay voices
        var ov_def = function (v) {
        ...
        var set_note = function (s, sf) {
            var i, m, note, p, pit, a, nn, delta = Jianpu.cgd2cde[sf + 7] - 2;
            s.stem = -1;
            s.stemless = true;
            if (s.sls) {
    ...
    Jianpu.prototype.draw_symbols = function (of, p_voice) {
        var s, s2, g, nl, y, C = abc2svg.C, dot = "\ue1e7", anno_a = abc.anno_a, staff_tb = abc.get_staff_tb(), out_svg = abc.out_svg.bind(abc), out_sxsy = abc.out_sxsy.bind(abc), xypath = abc.xypath.bind(abc);
        if (!p_voice.jianpu) {
            of(p_voice);
            return;
        }
        // draw the duration lines under the notes
        var draw_dur = function (s1, x, y, s2, n, nl) {
            var s, s3;
            xypath(x - 3, y + 5);
        ...
        var draw_dgr = function (s) {
            var g, x, l, y = staff_tb[s.st].y, s2 = s.extra, nl = s2.nflags;
        ...
        var out_mus = function (x, y, p) {
            out_svg('<text x="');
            out_sxsy(x, '" y="', y);
            out_svg('">' + p + '</text>\n');
        };
        var out_txt = function (x, y, p) {
            out_svg('<text class="fj" x="');
            out_sxsy(x, '" y="', y);
            out_svg('">' + p + '</text>\n');
        };
        var draw_hd = function (s, x, y) {
            var m, note, ym;
            for (m = 0; m <= s.nhd; m++) {
                note = s.notes[m];
                out_txt(x - 3.5, y + 8, "01234567-"[note.jn]);
                if (note.acc)
                    out_mus(x - 12, y + 12, Jianpu.acc_tb[note.acc + 2] || "");
                if (note.jo > 2) {
    ...
    Jianpu.prototype.set_fmt = function (of, cmd, param) {
        if (cmd == "jianpu") {
    ...
    Jianpu.prototype.set_pitch = function (of, last_s) {
        of(last_s);
        if (!last_s)
            return; // first time
        var C = abc2svg.C, s;
    ...
    Jianpu.prototype.set_vp = function (of, a) {
        var i, p_v = abc.curvoice;
    ...
    Jianpu.prototype.set_width = function (of, s) {
        of(s);
    ...
    Jianpu.hook = function (abc) {
        var jianpu = new Jianpu(abc);
        // Wrapping methods
    ...
    Jianpu.k_tb = ["Cb", "Gb", "Db", "Ab", "Eb", "Bb", "F",
        "C",
        "G", "D", "A", "E", "B", "F#", "C#"];
    Jianpu.cde2fcg = new Int8Array([0, 2, 4, -1, 1, 3, 5]);
    Jianpu.cgd2cde = new Int8Array([0, -4, -1, -5, -2, -6, -3,
        0, -4, -1, -5, -2, -6, -3, 0]);
    Jianpu.acc2 = new Int8Array([-2, -1, 3, 1, 2]);
    Jianpu.acc_tb = ["\ue264", "\ue260", "", "\ue262", "\ue263", "\ue261"];
    return Jianpu;
}());
exports.Jianpu = Jianpu;
