...
let abc: Abc;
export class Draw {
	acc_nd = {};  	// cache of the microtonal accidentals
	cache;
	anno_a = [];
	...
	b_pos(grace: boolean, stem: number, nflags: number, b: number): number {
		var top, bot, d1, d2,
			shift = !grace ? C.BEAM_SHIFT : 3.5,
			depth = !grace ? C.BEAM_DEPTH : 1.8
		...
		d1 = rnd6(top - C.BEAM_OFFSET);
		d2 = rnd6(bot + C.BEAM_OFFSET)
	...
	sym_dup(s: any): any {
		...
		delete s.a_dd;
		delete s.tp
		s.notes = Abc.clone(s.notes)
		for (m = 0; m <= s.nhd; m++) {
			note = s.notes[m] = Abc.clone(s.notes[m])
			delete note.a_dd
		}
		return s
	}
	...
	/* -- calculate a beam -- */
	calculate_beam(bm: any, s1: any): boolean {
		...
		st = s1.st;
		v = s1.v;
		stem_xoff = s1.grace ? C.GSTEM_XOFF : 3.5
		for (s2 = s1; ; s2 = s2.next) {
			...
			if (!s2.next) {		/* beam towards next music line */
				...
				delete s.sls;
				s.x += 12
				if (s.x < this.realwidth - 12)
					s.x = this.realwidth - 12;
				s2 = s;
				notes++
				break
			}
		...
		if (two_dir) {
			...
			if (n)				// same side
				...
				n = -(s1.nflags * s1.stem + s2.nflags * s2.stem)
					/ 2
			b += ((s1.grace ? 3.5 : C.BEAM_SHIFT) * n
				+ C.BEAM_DEPTH * s1.stem) / 2
		} else if (!s1.grace) {		/* normal notes */
			var beam_h = C.BEAM_DEPTH + C.BEAM_SHIFT * (nflags - 1)
			...
			for (; s && s.time <= s2.time; s = s.ts_next) {
				...
				if (s.v == v) {
					...
					stem_err += C.BEAM_DEPTH + C.BEAM_SHIFT * (s.nflags - 1)
				} else {
			...
			for (s = s1.next; ; s = s.next) {
				switch (s.type) {
					...
					case C.CLEF:
						...
						if (s1.stem > 0) {
							y = s.ymx - y
								+ C.BEAM_DEPTH + C.BEAM_SHIFT * (nflags - 1)
								+ 2
							if (y > 0)
								b += y
						} else {
		...
		for (s = s1; ; s = s.next) {
			switch (s.type) {
				...
				case C.REST:
					...
					dy = C.BEAM_DEPTH + C.BEAM_SHIFT * (nflags - 1)
						+ (s.head != C.FULL ? 4 : 9)
					if (s1.stem > 0) {
						y -= dy
						if (s1.multi == 0 && y > 12)
	...
	draw_beams(bm) {
		...
		function draw_beam(x1, x2, dy, h, bm,
			...
			x2 -= x1;
			x2 /= abc.stv_g.scale;
			dy2 = bm.a * x2 * abc.stv_g.stsc
			xypath(x1, y1, true);
			abc.output += 'l' + x2.toFixed(1) + ' ' + (-dy2).toFixed(1) +
				'v' + h.toFixed(1) +
	...
	draw_lstaff(x) {
		...
		function draw_sysbra(x, st, flag) {
			...
			yt = abc.staff_tb[st].y + abc.staff_tb[st].topbar
				* abc.staff_tb[st].staffscale;
			yb = abc.staff_tb[st_end].y + abc.staff_tb[st_end].botbar
				* abc.staff_tb[st_end].staffscale
			if (flag & (C.CLOSE_BRACE | C.CLOSE_BRACE2))
		...
		for (i = 0; ; i++) {
			...
			if (fl & (C.OPEN_BRACE | C.OPEN_BRACKET))
				l++
			if (cur_sy.st_print[i])
				break
			if (fl & (C.CLOSE_BRACE | C.CLOSE_BRACKET))
				l--
			if (i == nst)
				break
		}
		...
		if (i == j && l == 0)
			return
		yb = abc.staff_tb[j].y + abc.staff_tb[j].botbar * abc.staff_tb[j].staffscale;
		h = abc.staff_tb[i].y + abc.staff_tb[i].topbar * abc.staff_tb[i].staffscale - yb;
		xypath(x, yb);
		abc.output += "v" + (-h).toFixed(1) + '"/>\n'
		for (i = 0; i <= nst; i++) {
			fl = cur_sy.staves[i].flags
			if (fl & C.OPEN_BRACE)
				draw_sysbra(x, i, C.CLOSE_BRACE)
			if (fl & C.OPEN_BRACKET)
				draw_sysbra(x, i, C.CLOSE_BRACKET)
			if (fl & C.OPEN_BRACE2)
				draw_sysbra(x - 6, i, C.CLOSE_BRACE2)
			if (fl & C.OPEN_BRACKET2)
				draw_sysbra(x - 6, i, C.CLOSE_BRACKET2)
		}
	}
	...
	// (possible hook)
	sharp_cl = new Int8Array([24, 9, 15, 21, 6, 12, 18]);
	...
	draw_rest(s) {
		...
		if (s.notes[0].shhd)
			x += s.notes[0].shhd * abc.stv_g.scale
	...
	draw_mrest(s) {
		...
		s.x = (x1 + x2) / 2
		anno_start(s)
		if (!abc.cfmt.oldmrest || s.nmeas > abc.cfmt.oldmrest) {
			Asvg.out_XYAB('<path d="mX Y', x1 + .6, y - 2.7)
			abc.output += 'v2.7h-1.4v-10.8h1.4v2.7h'
				+ ((x2 - x1 - 2.8) / abc.stv_g.scale).toFixed(1)
				+ 'v-2.7h1.4v10.8h-1.4v-2.7z"/>\n'
		} else {
			omrest()		// old multirest
		}
		if (s.tacet)
			Asvg.out_XYAB('<text x ="X" y="Y" style="font-size:12px;font-weight:700"\
  text-anchor="middle">A</text>\n',
				s.x, y + 18, s.tacet)
		else
			Asvg.out_XYAB('<text x ="X" y="Y" text-anchor="middle">A</text>\n',
				s.x, y + 22, m_gl(p))
		anno_a.push(s)
	} // draw_mrest()
	...
	draw_basic_note(s, m, y_tb) {
		var i, p, yy, dotx, doty, inv, head, dots, nflags,
			...
			shhd = note.shhd * abc.stv_g.scale,
			x_note = x + shhd,
			y_note = y + staffb
		...
		if (dots && (s.dots || note.dur != s.dur)) {
			dotx = x + (6.6 + s.xmx) * abc.stv_g.scale
			if (y_tb[m] == undefined) {
	...
	draw_note(s,
		...
		var s2, i, m, y, slen, c, nflags,
			...
			x = s.x,
			x_st = s.x + note.shhd * abc.stv_g.scale,
			y = y_head(s, note),
		...
		if (fl && s.trem1) {
			...
			if (s.head == C.FULL || s.head == C.EMPTY) {
				x1 += (s.grace ? C.GSTEM_XOFF : 3.5) * s.stem
				if (s.stem > 0)
					slen += 6 + 5.4 * ntrem
				else
					slen -= 6 + 5.4
	...
	/* -- output a slur / tie -- */
	slur_out(x1, y1, x2, y2, dir, height, dotted) {
		var dx, dy, dz,
			alfa = .3,
			beta = .45;
		...
		//	var scale_y = abc.stv_g.st < 0 ? abc.stv_g.scale : 1
		var scale_y = 1			// (see set_dscale())
		if (!dotted)
			abc.output += '<path d="M'
		else
			abc.output += '<path class="stroke" stroke-dasharray="5,5" d="M';
		out_sxsy(x1, ' ', y1);
		abc.output += 'c' +
	...
	draw_slur(path,	// list of symbols under the slur
		...
		if (s_st2 && !recurr) {			// if not 2nd call to draw_slur()
			...
			for (i = 0; i < n; i++) {
				...
				path[i] = k
				if (k.dur) {
					k.notes = Abc.clone(k.notes)
					k.notes[0] = Abc.clone(k.notes[0])
		...
		if (a > C.SLUR_SLOPE || a < -C.SLUR_SLOPE) {
			a = a > C.SLUR_SLOPE ? C.SLUR_SLOPE : -C.SLUR_SLOPE
			if (a * dir > 0)
				y1 = y2 - a * (x2 - x1)
			else
				y2 = y1 + a * (x2 - x1)
		...
		if (k1.grace)
			x1 = k1.x - C.GSTEM_XOFF * .5
		if (k2.grace)
			x2 = k2.x + C.GSTEM_XOFF * .5
	...
	draw_tuplet(s1) {
		...
		if (!s2) {
			abc.error(1, s1, "No end of tuplet in this music line")
			return
		}
	...
	draw_sym_near() {
		...
		for (v = 0; v < voice_tb.length; v++) {
			...
			st = p_voice.st;
			//  if (st == undefined) {
			//abc.error(1, s, "BUG: no staff for voice " + p_voice.id)
			//    continue
			//  }
	...
	draw_systems(indent) {
		...
		function draw_staff(st, x1, x2) {
			...
			if (cache && cache.st_l == stafflines
				...
				&& cache.st_w == (w | 0)) {
				xygl(x1, staff_tb[st].y, 'stdef' + abc.cfmt.fullsvg)
				return
			}
			for (i = 0; i < l; i++, y -= il) {
				if (stafflines[i] == '.')
			...
			Asvg.out_XYAB('<g transform="translate(X, Y)">\n' + ln + '\n</g>\n', x1, y)
		} // draw_staff()
				...
				switch (bar_type[i]) {
					case "|":
						if (s.bar_dotted) {
							...
							Asvg.out_XYAB('<path class="bW" d="MX YvF"/>\n',
								x, bot, -h)
						} else {
							sb += 'M' + sx(x).toFixed(1)
					...
					default:
						...
						x -= 3;
						if (s.color)
							Asvg.out_XYAB('<path class="bthW" d="MX YvF"/>\n',
								x + 1.5, bot, -h)
						else
		...
		function out_bars() {
			...
			set_sscale(-1)
			if (sb)			// single bars
				abc.output += '<path class="bW" d="'
					+ sb
					+ '"/>\n'

			if (thb)		// thick bars [x, y, h]
				abc.output += '<path class="bthW" d="'
					+ thb
					+ '"/>\n'
		} // out_bars()
		...
		for (s = p_voice.sym; s; s = s.next) {
			...
			switch (s.type) {
				...
				case C.TEMPO:
					break
				default:
					abc.error(2, s, "draw_symbols - Cannot draw symbol " + s.type)
					break
			}
		}
	...
	draw_all_sym() {
		...
		function draw_sl2() {
			...
			while (1) {
				...
				abc.output += sl[3].slice(0, i)	// <path .. >d="M
