...
export class Music {
	...
	identify_note(s, dur_o) {
		...
		if (dur % 12 != 0)
			abc.error(1, s, "Invalid note duration $1", dur);
		dur /= 12			/* see C.BLEN for values */
		if (!dur)
			abc.error(1, s, "Note too short")
	...
	insert_clef(s, clef_type, clef_line) {
		var p_voice = s.p_v,
			new_s,
			st = s.st
	...
	public set_width(s) {
		...
		switch (s.type) {
			...
			case C.BAR:
				...
				if (abc.cfmt.measurenb > 0 && s.bar_num
					&& !(s.bar_num % abc.cfmt.measurenb))
					s.wr += 4
				return
			case C.CLEF:
			...
			case C.STAVES:
				break
			default:
				abc.error(2, s, "set_width - Cannot set width for symbol $1", s.type)
				break
		}
		s.wl = s.wr = 0
	...
	time2space(s, len) {
		...
		l = len - ((C.BLEN / 16 / 8) << i)
		space = abc.cfmt.spatab[i]
		if (l) {
			if (l < 0) {
				space = abc.cfmt.spatab[0] * len / (C.BLEN / 16 / 8)
			} else {
				if (i >= 9)
					i = 8
				space += (abc.cfmt.spatab[i + 1] - abc.cfmt.spatab[i]) *
					l / ((C.BLEN / 16 / 8) << i)
			}
		}
		return space
	...
	set_space(s, ptime) {
		...
		if (!len) {
			switch (s.type) {
				...
				//				if (s.nflags < -2)
				//					return abc.cfmt.spatab[0]
				//				return abc.cfmt.spatab[2]
				//			}
				//			break
			}
			return 0
	...
	set_repeat(s) {	// first note
		...
		if (n < 0) {				/* number of notes / measures */
			...
			for (s3 = s.prev; s3; s3 = s3.prev) {
				if (!s3.dur) {
					if (s3.type == C.BAR) {
						abc.error(1, s3, "Bar in repeat sequence")
						return
					}
					continue
				}
			...
			if (!s3) {
				abc.error(1, s, errs.not_enough_n)
				return
			}
			dur = s.time - s3.time;
			...
			for (s2 = s; s2; s2 = s2.next) {
				if (!s2.dur) {
					if (s2.type == C.BAR) {
						abc.error(1, s2, "Bar in repeat sequence")
						return
					}
					continue
				}
		...
		if (!s2) {
			abc.error(1, s, errs.not_enough_m)
			return
		}
	...
	set_nl(s) {			// s = start of line
		...
		function set_eol(s) {
			if (abc.cfmt.custos && voice_tb.length == 1)
				custos_add(s)
			s.nl = true
			s = s.ts_prev
			if (s.type != C.BAR)
			...
			for (s2 = s; s2 && s2.time == s.time; s2 = s2.ts_next) {
				switch (s2.type) {
					...
					case C.CLEF:
						...
						s3 = Abc.clone(s2)		// duplicate the K:/M:/clef
	...
	set_lines(s,		/* first symbol */
		...
		function ly_split(s, wmax) {
			...
			s2.a_ly = Abc.clone(s.a_ly)
			s2.a_ly[0] = Abc.clone(s.a_ly[0])
	...
	cut_tune(lwidth, lsh) {
		var s2, i, mc,
			pg_sav = {			// save the page parameters
				leftmargin: abc.cfmt.leftmargin,
				rightmargin: abc.cfmt.rightmargin,
				pagewidth: abc.cfmt.pagewidth,
				scale: abc.cfmt.scale
			},
		...
		if (abc.cfmt.indent && abc.cfmt.indent > lsh[1])
			indent += abc.cfmt.indent
		...
		for (; s; s = s.ts_next) {
			if (s.type == C.BLOCK) {
				switch (s.subtype) {
					...
					case "mc_start":
						mc = {
							lm: abc.cfmt.leftmargin,
							rm: abc.cfmt.rightmargin
						}
						break
					case "mc_new":
					case "mc_end":
						if (!mc)
							break
						abc.cfmt.leftmargin = mc.lm
						abc.cfmt.rightmargin = mc.rm
						abc.img.chg = 1 //true
						break
				}
			}
		...
		abc.cfmt.leftmargin = pg_sav.leftmargin
		abc.cfmt.rightmargin = pg_sav.rightmargin
		abc.cfmt.pagewidth = pg_sav.pagewidth
		abc.cfmt.scale = pg_sav.scale
		abc.img.chg = 1
		set_page()
	}
	...
	mrest_expand() {
		...
		function mexp(s) {
			...
			while (s2 && !s2.bar_type)
				s2 = s2.next
			if (!s2)
				return abc.error(1, s, "Lack of bar after multi-measure rest")
			bar = s2
	...
	set_auto_clef(st, s_start, clef_type_start) {
		var s, time, s2, s3,
			max = 14,				// "A,"
			min = 18				// "E"
				...
				switch (s.type) {
					...
					case C.METER:
						switch (s.type) {
							...
							case C.METER:
								s.p_v.meter = s
								insert_meter = abc.cfmt.writefields.indexOf('M') >= 0
									&& s.a_meter.length
								break
						}
						if (s.part)
	...
	set_global() {
		...
		if (glovar.ottava && abc.cfmt.sound != "play")
			set_ottava();
	...
	blocks = []		// array of delayed block symbols
	...
	public block_gen(s) {
		switch (s.subtype) {
			...
			case "mc_start":		// multicol start
				if (multicol) {
					abc.error(1, s, "No end of the previous %%multicol")
					break
				}
				multicol = {
					state: abc.parse.state,
					posy: posy,
					maxy: posy,
					lm: abc.cfmt.leftmargin,
					rm: abc.cfmt.rightmargin,
					w: abc.cfmt.pagewidth,
					sc: abc.cfmt.scale
				}
				break
			case "mc_new":			// multicol new
				if (!multicol || multicol.state != abc.parse.state) {
					abc.error(1, s, "%%multicol new without start")
					break
				}
				if (posy > multicol.maxy)
					multicol.maxy = posy
				abc.cfmt.leftmargin = multicol.lm
				abc.cfmt.rightmargin = multicol.rm
				abc.cfmt.pagewidth = multicol.w
				abc.cfmt.scale = multicol.sc
				posy = multicol.posy
				abc.img.chg = 1 //true
				break
			case "mc_end":			// multicol end
				if (!multicol || multicol.state != abc.parse.state) {
					abc.error(1, s, "%%multicol end without start")
					break
				}
				if (posy < multicol.maxy)
					posy = multicol.maxy
			...
			case "sep":
				...
				out_sxsy((abc.img.width - s.l) / 2 - abc.img.lm, ' ', 0)
				abc.output += 'h' + s.l.toFixed(1) + '"/>\n';
				vskip(s.sk2);
				break
			case "text":
				set_font(s.font)
	...
	public set_sym_glue(width) {
		...
		for (; s; s = s.ts_next) {
			...
			if (s.seqst) {
				xmin += s.shrink
				if (xmin > width) {
					if (abc.cfmt.singleline)
						width = xmin
					else
						abc.error(1, s, "Line too much shrunk $1 $2 $3",
							xmin.toFixed(1),
							xx.toFixed(1),
							width.toFixed(1))
					//				break
	...
	public output_music() {
		...
		if (abc.cfmt.singleline) {
			...
			v = abc.cfmt.singleline == 2	// if as wide as the page width
				? get_lwidth() : lwidth
			if (v > lwidth)
				lwidth = v
			else
				abc.img.width = lwidth * abc.cfmt.scale + abc.img.lm + abc.img.rm + 2
		} else {
		...
		while (1) {				/* loop per music line */
			...
			if (realwidth) {
				if (abc.img.wx < realwidth)
					abc.img.wx = realwidth
				if (indent) {
					abc.img.wx += indent
					posx += indent
				}
