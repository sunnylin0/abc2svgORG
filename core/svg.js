...
function sy(y) {
  ...
  if (stv_g.v >= 0) return (stv_g.dy - y) / stv_g.vsc;
...
function svg_flush() {
  if (multicol || !user.img_out || posy == 0) return;
  ...
  if (cfmt.fgcolor) head += ' color="' + cfmt.fgcolor + '"';
  font = get_font('music');
  head += ' class="' + font_class(font) + ' tune' + tunes.length + '"\n'; // tune index for play

  posy *= cfmt.scale;
  if (user.imagesize != undefined) head += user.imagesize;
