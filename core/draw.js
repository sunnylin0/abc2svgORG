...
var STEM_MIN = 16 /* min stem height under beams */,
  ...
  SLUR_SLOPE = 0.7, // max slope of a slur
...
//		if (!two_staves && !s1.grace) {
  ...
  if (s.grace && s1.fmt.flatbeams) a = 0;
	// if a note inside the beam is the closest to the beam, the beam is flat
  else if (!two_dir && notes >= 3 && s_closest != s1 && s_closest != s2) a = 0;

  y = s1.ys + staff_tb[st].y;
	if (a == undefined) {
    if (two_dir && s1.stem != s2.stem && s1.st == s2.st) {
      // if inverted stems,
	...
	if (a != 0) {
    a = (s1.fmt.beamslope * a) / (s1.fmt.beamslope + Math.abs(a)); // max steepness for beam
    if (a > -0.04 && a < 0.04) a = 0; // slope too low
	}
  ...
  s = s1;
    ...
    n = 1;
		while (1) {
      if (s.stem != s1.stem && (s.nflags == 1 || s.beam_br1 || s.beam_br2)) {
        n = 0;
        break;
			}
      if (s == s2) break;
    ...
    if (n)
      ...
      n =
        ...
        4; // different sides
    else n = -(s1.nflags * s1.stem + s2.nflags * s2.stem) / 2;
    b += ((s1.grace ? 3.5 : BEAM_SHIFT) * n + BEAM_DEPTH * s1.stem) / 2;
  } else if (!s1.grace) {
    /* normal notes */
  ...
  if (max_stem_err > 0)
	...
	for (s = s1.next; ; s = s.next) {
        ...
        case C.REST /* cannot move rests in multi-voices */:
          if (!s.multi) break;
          g = s.ts_next;
          if (!g || g.st != st || (g.type != C.NOTE && g.type != C.REST)) break;
...
//		case C.BAR:
		...
		case C.GRACE:
            ...
            y = a * g.x + b;
				if (s1.stem > 0) {
              y = g.ymx - y + BEAM_DEPTH + BEAM_SHIFT * (nflags - 1);
              if (y > 0) b += y;
				} else {
              y = g.ymn - y - BEAM_DEPTH - BEAM_SHIFT * (nflags - 1);
              if (y < 0) b += y;
				}
			}
          break;
    ...
    if (da != 0) bm.a += da;
      ...
      bsh = shift * beam_dir;
        ...
        bsh = osh * beam_dir;
				for (s = k1; ; s = s.next) {
					if (s.type == C.NOTE)
            s.ys = bm.a * s.xs + bm.b - staff_tb[s.st].y - bh;
          if (s == k2) break;
				}
			}
...
function draw_meter(s) {
  ...
  var i,
    ...
    if (
      s.a_meter[i + 1] &&
      (s.a_meter[i + 1].top == '|' || s.a_meter[i + 1].top == '.')
    ) {
...
Abc.prototype.draw_hl = function (s) {
  ...
  if (!p_staff.hll || s.invis) return; // no helper line (no line)
	for (i = 0; i <= s.nhd; i++) {
    note = s.notes[i];
  ...
  if (a_acc && !s.exp)
    ...
    var acc,
      ...
      shift =
        ...
        3;
      while (shift < -3)
        // let the accidentals inside the staff
        shift += 21;
      while (shift > 27) shift -= 21;
...
//dx = 6
  ...
  for (s2 = s.ts_next; s2 && !s2.seqst; s2 = s2.ts_next); // search the next symbol on the same staff
  x2 = s2 ? s2.x - 20 : s.x + 20;

  s.x = (x1 + x2) / 2;
  anno_start(s);
	if (!cfmt.oldmrest || s.nmeas > cfmt.oldmrest) {
    out_XYAB('<path d="mX Y', x1 + 0.6, y - 2.7);
    output +=
      'v2.7h-1.4v-10.8h1.4v2.7h' +
    ...
    if (s.tacet)
    out_XYAB(
      '<text x ="X" y="Y" style="font-size:12px;font-weight:700"\
...
//	y_set(s.st, !below, x0, x3 - x0, y0)
	if (below) {
    if (y0 < g.ymn) g.ymn = y0;
	} else {
    if (y0 > g.ymx) g.ymx = y0;
	}
} // grace_slur()
...
//		return
    ...
    p = 'custos';
        ...
        if (!tsnext && s.next && s.next.type == C.BAR && !s.next.next) dots = 0;
        x_note += 1;
        break;
		case C.EMPTY:
        p = 'Hd'; // white note
  ...
  if (note.color != undefined) old_color = set_color(note.color);
	if (p) {
		if (s.grace || inv) {
      if (s.grace) g_open(x_note, y_note, 0, 0.66, 0);
      else g_open(x_note, y_note, 0, 1, -1);
      x_note = y_note = 0;
		}
    if (!self.psxygl(x_note, y_note, p)) xygl(x_note, y_note, p);
    if (s.grace || inv) g_close();
	}
...
function draw_note(s, fl) {
  ...
  if (s.dots) setdoty(s, y_tb);
    ...
    if (!fl || nflags <= 0) {
      ...
      /* stem and flags */
      out_stem(x_st, y, slen, s.grace, nflags, s.fmt.straightflags);
		}
  } else if (s.xstem) {
    /* cross-staff stem */
		s2 = s.ts_prev;
	...
	/* draw the note heads */
  for (m = 0; m <= s.nhd; m++) draw_basic_note(s, m, y_tb);
}
...
function draw_slur(
  ...
  // recurrent call when slur on two staves
  var i,
    k,
    g,
    x1,
  ...
  if (s_st2 && !recurr) {
    // if not 2nd call to draw_slur()
    if (!gene.a_sl) gene.a_sl = [];
    ...
    if (s_st2.st > k1.st) h = -h;
		for (i = 0; i < n; i++) {
      k = path[i];
			if (k.st == k1.st) {
        if (k.dur) a = k; // (used for types // and \\)
        continue;
			}
      k = clone(k);
      if (path[i] == s_st2) s_st2 = k;
      path[i] = k;
			if (k.dur) {
        ...
        k.notes[0] = clone(k.notes[0]);
				if (sl.ty & C.SL_CENTER) {
					if (k.st != a.st) {
            sl.ty = (sl.ty & ~0x07) | (a.st < k.st ? C.SL_BELOW : C.SL_ABOVE);
            z = k1.ymn;
            h = k2.ymx;
						if (k.st < a.st) {
							for (i1 = 1; i1 < i; i1++) {
...
//						|| k2.ys < y2 + 3))
  ...
  if (k2.grace) x2 = k2.x + GSTEM_XOFF * 0.5;
...
//fixme: the following code seems better!
  addy = y1 - a * x1;
  if (height > 0) addy += 3 * Math.sqrt(height) - 2;
  else addy -= 3 * Math.sqrt(-height) - 2;
	for (i = 0; i <= i2; i++) {
    k = path[i];
    if (k.st != k1.st || k.type == C.BAR) continue;
    y = a * k.x + addy;
    if (k.ymx < y) k.ymx = y;
    else if (k.ymn > y) k.ymn = y;
    if (recurr)
      // no room when slur on 2 staves
      continue;
		if (i == i2) {
      dx = x2;
      if (sl.nte) dx -= 5;
    ...
    if (last && s2.time > last.time) return; // will be drawn next time
      ...
      case 'o': // no end
        ...
        s2 = s3;
          ...
          if (s3.st == s.st) s2 = s3;
          if (s3.ts_prev.time != s2.time) break;
			}
        break;
		}
...
function draw_tuplet(s1) {
  ...
  if (
    ...
    xm = (s2.x + s1.x) / 2;
    if (dir == C.SL_ABOVE)
      // 8 = width around the middle
      ym = y_get(stu, 1, xm - 4, 8);
    else ym = y_get(std, 0, xm - 4, 8) - gene.curfont.size;
    ...
    yy = ym + gene.curfont.size * 0.22;
    if (tp.f[2] == 0)
      // if 'which' == number
      xy_str(xm, yy, tp.p.toString(), 'c');
    else xy_str(xm, yy, tp.p + ':' + tp.q, 'c');
...
//fixme: s3 cannot be null (bar at end of staff)
	...
	if (s1.st >= s2.st) {
      ...
      y1 = ym > staff_tb[s1.st].topbar + 2 ? ym : staff_tb[s1.st].topbar + 2;
	} else {
      y1 = staff_tb[s1.st].topbar + 2;
	}
	if (s2.st >= s1.st) {
      ym = y_get(s2.st, 1, x2 - 4, 8);
      y2 = ym > staff_tb[s2.st].topbar + 2 ? ym : staff_tb[s2.st].topbar + 2;
	} else {
    ...
    dy = 0;
	for (s3 = s1; ; s3 = s3.next) {
      if (!s3.dur /* not a note or a rest */ || s3.st != stu) {
        if (s3 == s2) break;
        continue;
		}
		yy = ym + (s3.x - xm) * a;
    ...
    dy = 0;
	for (s3 = s1; ; s3 = s3.next) {
      if (!s3.dur /* not a note nor a rest */ || s3.st != std) {
        if (s3 == s2) break;
        continue;
		}
		yy = ym + (s3.x - xm) * a;
  ...
  if (dir == C.SL_ABOVE) y_set(stu, 1, xm - 3, 6, yy + 2);
  else y_set(std, 0, xm - 3, 6, yy);
}
...
function draw_tie(not1, not2, job) {
  ...
  y = staff_tb[st].y + 3 * (p - 18) + 0.8 * dir;
  h = (0.15 * (x2 - x1) + 14) * dir * s1.fmt.tieheight;
//	anno_start(k1, 'slur')
...
//	clef_chg = false
  ...
  for (; s1; s1 = s1.next) {
    ...
    if (
      ...
      not2 = not1.tie_e;
		if (!not2) {
        if (not1.tie_ty) draw_tie(not1, not1, 2);
        continue;
		}
      s2 = not2.s;
  ...
  var p_voice,
    ...
    k *= a;
		if (s1.stem > 0) {
			while (i <= j) {
        y = k * i + b;
        if (p_st.top[i] < y) p_st.top[i] = y;
        i++;
			}
		} else {
			while (i <= j) {
        y = k * i + b;
        if (p_st.bot[i] > y) p_st.bot[i] = y;
        i++;
			}
		}
	} // set_yab()
...
//						g.ymn - 1)
      ...
      ymn = s.ymn;
      if (
        s.notes[0].acc && // set room for the accidental
...
function set_staff() {
  ...
  var sy_staff_prev = sy.staves[prev_staff];
    ...
    while (!gene.st_print[++prev_staff]) staff_tb[prev_staff].y = -y;
		while (1) {
  ...
  mbot *= staff_tb[prev_staff].staffscale;
	...
	dy = -mbot;
  staffsep = fmt.staffsep * 0.5;
  if (dy < staffsep) dy = staffsep;
  maxsep = fmt.maxstaffsep * 0.5;
  if (dy > maxsep) dy = maxsep;
...
function draw_systems(indent) {
  var s,
    ...
    var st,
      ...
      l = stlines.length;
			for (i = 0; i < l; i++) {
        if (stlines[i] != '.' && stlines[i] != '-') break;
			}
      bar_bot[st] =
        staff_tb[st].y + 6 * (i == l - 1 ? l - 2 : i >= l - 2 ? l - 3 : i) * sc;
			if (!dy) {
        dy =
          staff_tb[st].y + 6 * (i == l ? l + 1 : i >= l - 2 ? l : l - 1) * sc;
			}
...
//					case ':':
    ...
    if (!i) return;
      ...
      b = ba[i];
      st = b[0].st;
      if (b[1] > bar_bot[st]) b[1] = bar_bot[st];
      if (b[2] < bar_height[st]) b[2] = bar_height[st];
      if (b[3] < bar_ng[st]) b[3] = bar_ng[st];
      if (b[0].seqst)
    ...
    if (
      ...
      dy = 0;
            ...
            continue;
				case ty:
            ln +=
              'm-' + w.toFixed(1) + ' ' + dy.toFixed(2) + 'h' + w.toFixed(1);
            dy = 0;
            continue;
				}
        if (ty != undefined) ln += '"/>\n';
        ty = stafflines[i];
        ln += '<path class="' + tycl[ty] + '" d="m0 ' + y + 'h' + w.toFixed(1);
        dy = 0;
			}
      ln += '"/>';
		}
    y = staff_tb[st].y;
    if (!cache && w > get_lwidth() - 10 && staff_tb[st].staffscale == 1) {
			cache = {
				st_l: stafflines,
        st_w: w | 0,
      };
...
//	 && (s.ts_prev.type != C.BAR || s.ts_prev.st != st - 1))
    ...
    if (bar_type == '||:') bar_type = '[|:';
        ...
        case '|':
            ...
            w =
              top / 6 <= 9
                ? [0, 0, 4, 3.6, 4.8, 4.3, 4, 4.7, 4.4, 4.9][top / 6]
                : 5;
					out_XYAB(
			'<path class="bW" stroke-dasharray="A,A" d="MX YvG"/>\n',
              x,
              bot,
              w * p_staff.staffscale,
              -h,
            );
				} else if (s.color) {
...
//			case "]":
    ...
    if (i < 6)
      // no ledger line if rest smaller than minim
      return;

    if (i == 7 && y == 12 && stlines.length <= 2) y -= 6; // semibreve a bit lower
    ...
    do {
      // search a voice of this staff
      s = s.ts_next;
    } while (s.st != st);
    while (
      s.prev && // search the first symbol of this voice
      s.prev.time >= tim
    )
      s = s.prev;
    if (s.bar_type) return s.x;
    return s.x - s.wl;
	} // st1()
	...
	for (s = tsfirst; s; s = s.ts_next) {
        ...
        sy = s.sy;
          ...
          if (
            ...
            x2 = s.ts_prev.x;
            if (
              sy.staves[st].stafflines.length >
      ...
      case C.BAR: // display the bars after the staves
        if (s.invis || !s.bar_type || !cur_sy.st_print[s.st]) break;
        if (
          s.second &&
          (!s.ts_prev || (s.ts_prev.type == C.BAR && s.ts_prev.st == s.st))
        )
          break;
        ...
        break;
		case C.GRACE:
        for (s2 = s.extra; s2; s2 = s2.next) self.draw_hl(s2);
        break;
		case C.NOTE:
        if (!s.invis) self.draw_hl(s);
        break;
		case C.REST:
        if (
          s.fmr || // if full measure rest
          (s.rep_nb && s.rep_nb >= 0)
        )
          center_rest(s);
        if (!s.invis) hl_rest(s);
        break;
//		default:
...
//	bm.s2 = undefined
	for (s = p_voice.sym; s; s = s.next) {
		if (s.invis) {
			switch (s.type) {
			case C.CLEF:
          if (s.time >= staff_tb[s.st].clef.time) staff_tb[s.st].clef = s;
          continue;
			case C.KEY:
          p_voice.ckey = s;
			default:
...
//--fixme: recall set_scale if different staff
		...
		case C.CLEF:
          ...
          if (s.x == staff_tb[st].clef.x && s.v != staff_tb[st].clef.v) break;
          staff_tb[st].clef = s;
			}
        if (!gene.st_print[st]) break;
			set_color();
...
//						y += 1
          ...
          xygl(
            x - 2,
            y,
            s.clef_octave == 7 || s.clef_octave == -7 ? 'oct' : 'oct2',
          );
			}
        anno_a.push(s);
        break;
...
function set_tie_dir(s) {
  ...
  for (; s; s = s.next) {
    ...
    pit = 128;
      ...
      dir =
        pit >= 22 // up if above middle staff
          ? C.SL_ABOVE
          : C.SL_BELOW;
    else if (s.multi) dir = s.multi > 0 ? C.SL_ABOVE : C.SL_BELOW;
    else dir = s.stem < 0 ? C.SL_ABOVE : C.SL_BELOW;
