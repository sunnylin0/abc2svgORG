...
function acc_shift(notes, grh) {
	...
	for (i = n - 1; --i >= 0;) {
		...
		dx = (grh || (notes[i].s ? dx_tb[notes[i].s.head] : 9)) - dx;
		ps = notes[i].pit;
		for (i1 = n; --i1 >= 0;) {
...
function set_acc_shft() {
	...
	while (s) {
		...
		for (; s != s2; s = s.ts_next) {
			if (!s.invis) Array.prototype.push.apply(notes, s.notes);
		}
...
function lktsym(s, next) {
	...
	s.seqst =
		...
		(w_tb[s.ts_prev.type] != w_tb[s.type] && !!w_tb[s.ts_prev.type]);
	if (!next || next.seqst) return;
	next.seqst =
		next.time != s.time || (w_tb[s.type] != w_tb[next.type] && !!w_tb[s.type]);
	if (next.seqst) {
		old_wl = next.wl;
		self.set_width(next);
		if (next.a_ly) ly_set(next);
		if (!next.shrink) {
			next.shrink = next.wl;
			if (next.prev) next.shrink += next.prev.wr;
		} else {
...
function set_graceoffs(s) {
	...
	for (; ; g = g.next) {
		...
		dx = 0;
		for (m = g.nhd; m >= 0; m--) {
			if (g.notes[m].shac - 2 > dx) dx = g.notes[m].shac - 2;
		}
		x += dx;
		g.x = x;

		if (g.nflags <= 0) g.beam_st = g.beam_end = true;
		next = g.next;
		if (!next) {
			g.beam_end = true;
			break;
	...
	for (; s; s = s.ts_next) {
		...
		for (i = 0; i < s.a_gch.length; i++) {
			...
			if (w0) {
				...
				if (w0 > x + ch.x) {
					if (
						s.prev && // (if not at start of a secondary voice)
						s.prev.seqst &&
						s.prev.type == C.BAR
					)
						// don't move away
...
Abc.prototype.set_width = function (s) {
	...
	switch (s.type) {
		...
		case C.REST:
			...
			if (s2) {
				switch (s2.type) {
					...
					case C.NOTE:
						...
						if (s.stem * s2.stem < 0) wlnote += s.stem < 0 ? 3 : -3;
						break;
				}
			}
			...
			for (m = 0; m <= s.nhd; m++) {
				...
				if (
					...
					tmp = nt.shac + (typeof acc == 'object' ? 5.5 : 3.5);
					if (s2 && s2.stem < 0 && s2.notes[s2.nhd].pit > nt.pit + 4) tmp -= 5;
					if (wlnote < tmp) wlnote = tmp;
				}
				if (nt.a_dd) {
					// if decoration in chord
					tmp = deco_wch(nt);
					if (w < tmp) w = tmp;
				}
			}
			...
			if (s.a_dd) wlnote = deco_width(s, wlnote);
			...
			if (s2) {
				switch (s2.type) {
					...
					// fall thru
					case C.KEY:
						if (s.a_gch) wlw += 4; // have some room for the chord symbols
					// fall thru
					case C.METER:
						wlw += 3;
						break;
		...
		case C.BAR:
			...
			/* if preceeded by a grace note sequence, adjust */
			s2 = s.prev;
			if (s2 && s2.type == C.GRACE) s.wl -= 6;
			for (; s2; s2 = s2.prev) {
				if (w_tb[s2.type]) {
					if (s2.type == C.STBRK) s.wl -= 12;
					break;
				}
			}

			if (s.a_dd) s.wl = deco_width(s, s.wl);
		...
		case C.KEY:
			...
			if (!n1) break; // no width
			s.wr = 5.5 * n1 + esp;
			if (s.prev && !s.prev.bar_type) s.wl += 2;
			return;
		case C.METER:
			s.x_meter = [];
			if (!s.a_meter.length) break; // no width
			wlw = 0;
	...
	if (s.seqst && s.ts_next) s.ts_next.seqst = 0;
};
...
function time2space(s, len) {
	...
	if (l) {
		if (l < 0) {
			...
			if (i >= 9) i = 8;
			space +=
				((cfmt.spatab[i + 1] - cfmt.spatab[i]) * l) / ((C.BLEN / 16 / 8) << i);
		}
	}
	return space;
}
...
function set_sp_tup(s, s_et) {
	...
	while (1) {
		...
		if (s2.seqst) {
			wsp += s2.space;
			if (s2.bar_type) wsp += 10; // (fixme: not exact)
		}
		if (s2 == s_et) break;
	}
...
function add_end_bar(s) {
	...
	if (s.next)
		// (must not be the end of the voice)
		s.next.prev = b;
	//	if (s.ts_next)
	s.ts_next.ts_prev = b;
	s.next = s.ts_next = b;
...
function set_allsymwidth(first) {
	...
	while (1) {
		itup = 0;
		do {
			if ((s.a_gch || s.a_ly) && !s_chs) s_chs = s;
			self.set_width(s);
			st = s.st;
...
function set_repeat(s) {
	...
	if (n == 1) i = k; /* repeat number */
	else i = n; /* check only 2 measures */
	for (s2 = s; s2; s2 = s2.next) {
		if (s2.type == C.BAR) {
			if (--i <= 0) break;
		}
...
function set_nl(s) {
	...
	function bardiv(so) {
		...
		function eol_bar(
			...
			for (s1 = so.ts_prev; s1.time == s.time; s1 = s1.ts_prev) {
				if (s1.v != s.v) continue;
				if (s1.bar_type) {
					if (s1.bar_type != '|') return; // don't change
					s2 = s1; // last symbol in previous line
					break;
				}
				if (!s3) s3 = s1.next; // possible anchor for the new bar
			}
			if (!s2) {
		...
		while (s.ts_prev && s.ts_prev.time == so.time) {
			...
			else if (!s1 && s.type == C.GRACE && s.seqst) so = s; // if grace note after a bar
			// move the start of line
		}
		...
		while (1) {
			...
			s2.ts_next = so;
			so.ts_prev = s2;
			if (s1 == s) s2.seqst = 1; //true
			s2.next = s1;
			if (s2.prev) s2.prev.next = s2;
			s1.prev = s2;
			if (s1.rbstop) s2.rbstop = s1.rbstop;
			if (s1.text) {
				s1.invis = 1; //true
				delete s1.xsh;
				delete s2.text;
	...
	function do_warn(s) {
		...
		for (s2 = s; s2 && s2.time == s.time; s2 = s2.ts_next) {
			switch (s2.type) {
				case C.KEY:
					if (!s2.fmt.keywarn || s2.invis) continue;
					for (s1 = s.ts_prev; s1; s1 = s1.ts_prev) {
						if (s1.type != C.METER) break;
					}
				// fall thru
				case C.METER:
					if (s2.type == C.METER) {
						if (!s.fmt.timewarn) continue;
						s1 = s.ts_prev;
					}
				// fall thru
				case C.CLEF:
					...
					if (s2.type == C.CLEF) {
						if (s2.clef_none)
							// if 'K: clef=none' after bar
							break;
						for (s1 = s.ts_prev; s1; s1 = s1.ts_prev) {
							switch (s1.type) {
								case C.BAR:
									if (s1.bar_type[0] == ':') break;
								// fall thru
								case C.KEY:
								case C.METER:
									continue;
	...
	for (; ; s = s.ts_next) {
		...
		switch (s.type) {
			case C.STAVES:
				...
				while (s.ts_next) {
					if (w_tb[s.ts_next.type] && s.ts_next.type != C.CLEF) break;
					s = s.ts_next;
				}
			...
			case C.BAR:
				...
				continue;
			case C.STBRK:
				if (!s.stbrk_forced) unlksym(s); /* remove */
				else done = -1; // keep the next symbols on the next line
				continue;
			case C.CLEF:
				if (done) break;
				continue;
...
function get_width(s, next) {
	...
	while (s != next) {
		if (s.seqst) {
			...
			else w += shrink * s.fmt.maxshrink + space * sp_fac;
			s.x = w;
		}
		s = s.ts_next;
	}
				...
				for (; s != next; s = s.ts_next) {
					if (s.seqst) {
						...
						if (
							...
							!s.in_tuplet &&
							(xmid - s5.x > x - xmid ||
								(s.time - bar_time) % (C.BLEN / 4) == 0)
						)
							s3 = s;
					}
					if (s.beam_st) beam |= 1 << s.v;
					if (s.beam_end) beam &= ~(1 << s.v);
					s5 = s; // start of new time sequence
				}
				if (s3) {
					do {
						// cut on the previous sequence
						s3 = s3.ts_prev;
					} while (!s3.seqst);
				}
			}
...
function cut_tune(lwidth, lsh) {
	...
	if (cfmt.indent && cfmt.indent > lsh[1]) indent += cfmt.indent;
	...
	for (; s; s = s.ts_next) {
		if (s.type == C.BLOCK) {
			switch (s.subtype) {
				...
				case 'scale':
				case 'staffwidth':
					if (!s.soln) self.set_format(s.subtype, s.param);
					break;
				case 'mc_start':
					mc = {
						lm: cfmt.leftmargin,
		...
		if (!s.ts_next) {
			...
			if (s.time == s2.time) continue; // empty music line!
			while (!s.seqst) s = s.ts_prev;
		}
		set_page();
...
function set_ottava() {
	...
	function deco_rm(s) {
		for (var i = s.a_dd.length; --i >= 0;) {
			if (s.a_dd[i].name.match(/1?[85][vm][ab]/)) s.a_dd.splice(i, 1);
		}
	} // deco_rm()
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		if (o) {
			...
			s1 = s;
			while (s1 && !s1.seqst) s1 = s1.ts_prev;
			if (s1) {
				// update the previous symbols
				while (s1 != s) {
					if (s1.st == st) {
						if (o[1]) sym_ott(s1, -staff_d[st]);
						if (o[0]) sym_ott(s1, -o[0] * 7);
					}
					s1 = s1.ts_next;
				}
			}
		...
		if (staff_d[st]) sym_ott(s, staff_d[st]);
	}
}
...
function mrest_expand() {
	...
	function mexp(s) {
		...
		while (s2 && !s2.bar_type) s2 = s2.next;
		if (!s2) return error(1, s, 'Lack of bar after multi-measure rest');
		bar = s2;
		while (!s2.bar_num)
			// get the bar number
			s2 = s2.ts_prev;
		...
		while (--nb > 0) {
			...
			if (s3) {
				if (s3.bar_type) s3.seqst = 0; //false
				lktsym(s2, s3);
				if (s3.type == C.BAR) delete s3.bar_num;
			} else {
				s3 = s;
			...
			s2.bar_type = '|';
			if (s4.bar_num && !s4.seqst) delete s4.bar_num;
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		if (!s.seqst && w_tb[s.ts_prev.type]) {
			...
			s2 = s.ts_next;
			while (!s2.seqst) {
				if (s2.type != C.MREST || s2.nmeas != s.nmeas) break;
				s2 = s2.ts_next;
			}
		}
		if (!s2.seqst) {
...
function set_auto_clef(st, s_start, clef_type_start) {
	...
	for (s = s_start; s; s = s.ts_next) {
		...
		if (s.notes[s.nhd].pit > max) max = s.notes[s.nhd].pit;
	}

	if (min >= 19 /* upper than 'F' */ || (min >= 13 && clef_type_start != 'b'))
		/* or 'G,' */
		return 't';
	if (max <= 13 /* lower than 'G,' */ || (max <= 19 && clef_type_start != 't'))
		/* or 'F' */
		return 'b';
	...
	for (s = s_start; s != s_last; s = s.ts_next) {
		...
		if (clef_type == 't') {
			if (s.notes[0].pit > 12 /* F, */ || s.notes[s.nhd].pit > 20) {
				/* G */
				if (s.notes[0].pit > 20 || s.notes[s.nhd].pit > 20) s_last_chg = s;
				continue;
			}
			s2 = s.ts_prev;
			if (
			...
			if (s.notes[0].pit <= 12 /* F, */ || s.notes[s.nhd].pit < 20) {
				/* G */
				if (s.notes[s.nhd].pit <= 12 || s.notes[0].pit <= 12) s_last_chg = s;
				continue;
			}
			s2 = s.ts_prev;
			if (
			...
			case C.STAVES:
				...
				for (v = 0; v < voice_tb.length; v++) {
					...
					if (!staff_clef[st].clef) {
						// new staff
						if (s2.clef_auto) {
							if (s2.clef_type != 'a') p_voice.clef = clone(p_voice.clef);
		...
		for (; s != u; s = s.ts_next) {
			...
			switch (s.type) {
				...
				case C.REST:
					...
					if (s.p_v.scale != 1 && s.ts_next.p_v.scale > s.p_v.scale) unlksym(s);
					else unlksym(s.ts_next);

					if (
						(!s.ts_prev.dur ||
							s.ts_prev.time != s.time ||
							s.ts_prev.st != s.st) &&
						(s.ts_next.time != s.time || s.ts_next.st != s.st)
					)
			...
			if (i == st_v.length - 1) {
				...
				if (i && i + 2 == st_v.length) {
					if (st_v[i].ymn - s.fmt.stemheight >= st_v[i + 1].ymx) s.multi = -1;
...
function set_rest_offset() {
	...
	function loffs(d) {
		return d > 0 ? Math.ceil(d / 6) * 6 : -Math.ceil(-d / 6) * 6;
	} // loffs()
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		switch (s.type) {
			...
			case C.REST:
				...
				if (s.prev && s.prev.type == C.NOTE)
					d =
						s.next && s.next.type == C.NOTE
							? (s.prev.y + s.next.y) / 2
							: s.prev.y;
				else if (s.next && s.next.type == C.NOTE) d = s.next.y;
				else if (s.prev && s.prev.type == C.REST) d = s.prev.y;
				if (s.multi > 0) {
					if (d >= 12) v_s.d = d - s.y;
				} else {
					if (d <= 12) v_s.d = d - s.y;
				...
				if (s.fmr)
					// if full meeasure rest
					v_s.end_time -= s.p_v.wmeasure * 0.3;
				if (s.seqst) continue;
				s2 = s.ts_prev;
				if (s2.st != s.st || s2.invis) continue;
				d = s2.ymn;
				...
				if (s2.type == C.REST && s2.y < 18 && s.y >= 6) v_s.d = (d - s.ymx) / 2;
				break;
			case C.NOTE:
				if (s.invis || !s.multi) continue;
				break;
		...
		for (v = 0; v < v_s_tb.length; v++) {
			...
			if (sy.voices[v].range > sy.voices[s.v].range) {
				if (s2.ymx + v_s.d <= s.ymn) continue;
				d = s.ymn - s2.ymx; // rest must go down
				//				if (s2.time < s.time) {
				if (s.type == C.REST) {
					if (!v_s_tb[s.v]) v_s_tb[s.v] = { d: 0 };
					if (v_s_tb[s.v].d < 6) v_s_tb[s.v].d = 6;
					d = -6;
				} else {
					d /= 2;
					if (s2.fmr) d -= 6;
				...
				if (s2.ymn + v_s.d >= s.ymx) continue;
				d = s.ymx - s2.ymn; // rest must go up
				if (
					s.type == C.REST && // if rest
...
function init_music_line() {
				...
				case C.METER:
					switch (s.type) {
						...
						case C.METER:
							s.p_v.meter = s;
							insert_meter =
								cfmt.writefields.indexOf('M') >= 0 && s.a_meter.length;
							break;
					}
					if (s.part) s.next.part = s.part;
					unlksym(s);
				// fall thru
				case C.TEMPO:
				case C.BLOCK:
	...
	for (v = 0; v < nv; v++) {
		...
		else if (!s.clef_none) delete s.invis;
		s.fmt = fmt;
	}
	...
	if (
		s.ts_next && // a bit further in case different keys per voice
		s.ts_next.type != C.CLEF && // (the clef may move in allsymwidth)
		!s.tp && // (start of a tuplet)
		!s.ts_next.a_ly
	)
		// (don't update next .shrink)
		...
		switch (s.type) {
			...
			case C.GRACE:
				for (s2 = s.extra; s2; s2 = s2.next) {
					...
					s2.dots = res[1];
					s2.nflags = res[2];
					if (s2.trem2 && (!s2.next || s2.next.trem2)) trem_adj(s2);
				}
				break;
			case C.NOTE:
			case C.REST:
...
function get_lshift() {
	...
	while (sy) {
		for (st = 0; st <= sy.nstaff; st++) {
			if (sy.staves[st].flags & (OPEN_BRACE2 | OPEN_BRACKET2)) {
				w = 12;
				break;
			}
			if (sy.staves[st].flags & (OPEN_BRACE | OPEN_BRACKET)) w = 6;
		}
		if (w == 12) break;
		sy = sy.next;
	}
...
function set_indent(lsh) {
	...
	// name or subname?
	if (fmt.systvoices) {
		// put the voice names in the staff system
		for (v = voice_tb.length; --v >= 0;) {
			p_voice = voice_tb[v];
			if (!cur_sy.voices[v] || !gene.st_print[p_voice.st]) continue;
			if (
				p_voice.nm &&
				(fmt.systvoices == 1 || (p_voice.new_name && fmt.systvoices == 3))
			) {
				vnt = 1; // full name
				break;
			}
			if (p_voice.snm) vnt = 2; // subname
		}
	}
...
function set_beams(sym) {
	...
	for (s = sym; s; s = s.next) {
		...
		if (!s.stem && s.multi) s.stem = s.multi;
		if (!s.stem) {
			// if note alone on the staff
			mid_p = s.mid / 3 + 18;
			...
			if (beam) {
				...
				if (n < mid_p) s.stem = 1;
				else if (n > mid_p || s.fmt.bstemdown) s.stem = -1;
...
function unison_acc(s1, s2, i1, i2) {
	...
	if (!acc) {
		...
		s2.xmx += d;
		if (s1.notes[i1].acc) s1.notes[i1].shac -= 7;
...
function set_overlap() {
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		if (
			d < -3 && // no clash if no dots clash
			(s2.notes[0].pit & 1 ||
				!(s1.dots || s2.dots) ||
				(!(s1.notes[s1.nhd].pit == s2.notes[0].pit + 2 && s1.dot_low) &&
					!(s1.notes[s1.nhd].pit + 2 == s2.notes[0].pit && s2.dot_low)))
		)
			continue;
		...
		while (1) {
			...
			switch (dp) {
				case 2:
					if (!(s1.notes[i1].pit & 1)) s1.dot_low = false;
					break;
				case 1:
					if (s1.notes[i1].pit & 1) s2.dot_low = true;
					else s1.dot_low = false;
					break;
				case 0:
					...
					break;
				case -1:
					if (s1.notes[i1].pit & 1) s2.dot_low = false;
...
Abc.prototype.set_stems = function () {
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		if (s.decstm != null) {
			// if deco on the stem
			if (nflags <= 0) {
				if (slen < s.decstm + 6) slen = s.decstm + 6;
			} else {
				var t = nflags * 4; // beams

				if (s.beam_st & s.beam_end) t += 2; // flags
				if (slen < s.decstm + 4 + t) slen = s.decstm + 4 + t;
			}
		}
		if (s.stemless) {
			if (s.stem >= 0) {
	...
	for (s = tsfirst; s; s = s.ts_next) {
		...
		switch (s.type) {
			...
			case C.BLOCK:
				if (!s.play) {
					blocks.push(s);
					unlksym(s);
				} else if (s.ts_next && s.ts_next.shrink) s.ts_next.shrink = 0;
				continue;
		}
		st = s.st;
		if (st > nstaff) {
		...
		switch (s.type) {
			...
			case C.MREST:
				...
				if (s.invis) continue;
				if (sy.staves[st].staffnonote || s.type == C.NOTE) break;
				continue;
		}
...
Abc.prototype.set_sym_glue = function (width) {
	...
	if (ll) {
		if ((xx - xx0 + xs) / width > 1 - s.fmt.stretchlast) stretch = 1; //true
	...
	if (xmin >= width) {
		...
		if (stretch) {
			...
			spf = spf_last;
			if (ll && spf < s.fmt.stretchlast) spf = s.fmt.stretchlast;
			else if (!ll) spf = 1 - cfmt.maxshrink * 0.75;
			if (spf > (width - xs) / xx) spf = (width - xs) / xx;
		}
		while (--cnt >= 0) {
			xx = 0;
			xse = 0;
...
Abc.prototype.output_music = function () {
	...
	if (cfmt.singleline) {
		...
		v =
			cfmt.singleline == 2 // if as wide as the page width
				? get_lwidth()
				: lwidth;
		if (v > lwidth) lwidth = v;
		else img.width = lwidth * cfmt.scale + img.lm + img.rm + 2;
	} else {
	...
	spf_last = 0.65; // last spacing factor
	while (1) {
		/* loop per music line */
		set_piece();
		...
		if (realwidth) {
			if (img.wx < realwidth) img.wx = realwidth;
			if (indent) {
				img.wx += indent;
				posx += indent;
			}
