...
function get_sym(p, cont) {
  ...
  i = 0;
    ...
    while (s && s.type != C.NOTE) s = s.next;
		if (!s) {
...
function get_lyrics(p, cont) {
  ...
  if (curvoice.ignore) return;
  if ((curvoice.pos.voc & 0x07) != C.SL_HIDDEN) curvoice.have_ly = true;
  ...
  if (cont) {
    ...
    if (gene.deffont != dfnt) {
      // if vocalfont change
      if (gene.curfont == gene.deffont) gene.curfont = dfnt;
      gene.deffont = dfnt;
		}
	} else {
    set_font('vocal');
    ...
    j = parse.istart + i + 2; // start index
        ...
        word = '';
          ...
          if (!p[i]) break;
              ...
              word += p[i++];
              c = p[i];
              if (c == '0') gene.curfont = gene.deffont;
              else if (c >= '1' && c <= '9') gene.curfont = get_font('u' + c);
					// fall thru
				default:
              word += p[i++];
              continue;
...
function draw_lyric_line(p_voice, j, y) {
  ...
  for (; s; s = s.next) {
    ...
    if (ly.font != gene.curfont) /* font change */ gene.curfont = ly.font;
		p = ly.t;
    ln = ly.ln || 0;
    w = p.wh[0];
    shift = ly.shift;
...
function draw_lyrics(p_voice, nly, a_h, y, incr) {
  ...
  if (incr > 0) {
    /* under the staff */
    if (y > -tsfirst.fmt.vocalspace) y = -tsfirst.fmt.vocalspace;
    y *= sc;
		for (j = 0; j < nly; j++) {
			y -= a_h[j] * 1.1;
      draw_lyric_line(p_voice, j, y + a_h[j] * 0.22); // (descent)
		}
    return y / sc;
	}
  ...
  y *= sc;
  for (j = nly; --j >= 0; ) {
    draw_lyric_line(p_voice, j, y + a_h[j] * 0.22);
    y += a_h[j] * 1.1;
	}
  return y / sc;
}
...
/*fixme:should get the real width*/
    ...
    nly_tb[v] = nly;
    if (nly == 0) continue;
    if (p_voice.pos.voc) above_tb[v] = (p_voice.pos.voc & 0x07) == C.SL_ABOVE;
    else if (
      voice_tb[v + 1] &&
